...
module.exports = class Context {
  ...
  printRoom(room) {
    const users = room.users.filter((u) => {
      return u.id !== this.user.id // User is not you.
          && this.state.users[u.id] // User exists in the state (i.e. has logged on since server restart).
          && this.state.users[u.id].online !== false; // User is currently online.
    });
